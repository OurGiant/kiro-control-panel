...
/**
 * Builds {@link HttpClient}s for outbound HTTPS calls (MCP catalog refresh,
 * update check). On Windows, trusts both the JDK's bundled cacerts and the
 * OS certificate store -- jpackage bundles its own JRE with its own cacerts,
 * completely separate from the Windows store, so a machine that's required
 * to go through a TLS-inspecting corporate proxy (custom root/intermediate
 * CA installed into Windows via GPO, same store the browser already trusts)
 * fails every outbound HTTPS call with a PKIX/SSLHandshakeException that the
 * JDK's default trust manager alone can never satisfy. See #44.
 */
