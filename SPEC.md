...
**Bundled by default, refreshable on demand (see issue #26 below for the
live-fetch path added later).** `src/main/resources/mcp-catalog.json` is
...
`version.properties`/`AppVersion` already use. The generator initially assumed the deep
...
## MCP catalog auto-refresh (issue #26, companion to issue #27)

Closes a gap in issue #11's "bundled, not live-fetched" design: the
bundled `mcp-catalog.json` only gets fresher when the *app* ships a new
release, but this is a packaged desktop app that can go weeks between
updates — so even with issue #27's scheduled regeneration keeping the
*repo* current, an already-installed copy would never see it. A "Refresh"
button in the MCP catalog dialog fetches the current `mcp-catalog.json`
straight from GitHub's raw-content URL for `main`
(`raw.githubusercontent.com/OurGiant/kiro-control-panel/main/src/main/resources/mcp-catalog.json`)
at runtime, decoupling catalog freshness from app release cadence
entirely — once a regenerated catalog merges to `main`, it's live for
every installed copy immediately, not just the next release.

`McpCatalogService.fetchFromUrl(String url)` mirrors `UpdateChecker
.fetchLatestRelease()`'s exact conventions verbatim (5s connect / 10s
request timeout, fresh `HttpClient` per call, synchronous `send`, one
broad `try/catch` collapsing every failure mode — offline, non-200,
malformed JSON — to a single `Optional.empty()`, `logger.debug` not
warn/error). Extended `McpCatalogService` itself rather than adding a new
class: a second acquisition strategy (URL vs. classpath) for the same
`List<McpCatalogEntry>` output is the same concern `load()` already owns,
not a new one — the closer precedent than the single-purpose utility
classes (`DesktopUtils`/`KiroSessionLauncher`/`ProcessDetacher`) is
`McpConfigService`, which already has both `load*` and `save*` on one
class. The shared parse step (`parseEntries(InputStream)`) was extracted
out of the existing `readFromResource()` so both the classpath and URL
paths deserialize identically — a real DRY win, not just a refactor for
its own sake.

`McpCatalogDialog`'s button-and-status wiring is `AboutDialog
.startUpdateCheck`'s `SwingWorker` shape verbatim (`doInBackground()` = one
service call, `done()` = `try { get() } catch (Exception e) { ...passive
status text...; return; }` then branch on the result) — this app's only
prior `SwingWorker` precedent, extended with the button disable/re-enable
`AboutDialog` doesn't need (its check auto-runs on dialog open; this is
the first *button-triggered* `SwingWorker` in the app). A failed refresh
never touches the currently-shown list or shows a blocking dialog — just
a passive "Could not refresh catalog" status text, matching the
established convention for background-fetch failures throughout this app.

**In-memory only for v1, not persisted to disk.** `McpCatalogDialog` is
already constructed fresh on every "Browse Catalog..." click, so
"resets to bundled on reopen/restart" is already the zero-code default
behavior. A disk cache would need a new cache-location convention
(nothing like it exists anywhere in this app yet), a staleness policy, and
a "prefer cache over classpath resource" branch in `load()` — real new
surface area for a ~39-entry JSON blob that fetches in well under a
second against GitHub's CDN. Left as a possible future enhancement, not
built now.

**Verified against the real production URL**, not just unit-tested: a
driven-UI harness clicked the actual Refresh button and confirmed the
status label reached "Catalog updated" and the button re-enabled, against
the live `raw.githubusercontent.com` endpoint — the strongest available
proof this actually works end-to-end, short of a real user's own machine.
`fetchFromUrl`'s network-calling wrapper itself is deliberately left
unit-untested, matching `UpdateCheckerTest`'s own precedent (it tests only
`isNewerVersion`, never `fetchLatestRelease()` — no HTTP-mocking library
in this project). `parseEntries` gets real unit test coverage instead
(valid JSON parses correctly, malformed JSON throws) since extracting it
was already a DRY win independent of testability.
